...
class BaseService(object):
    ...
    def _restart(self):
        if self.retry > self.max_retry:
            logger.info("Service start failed, exit: {}".format(self.name))
            self.EXIT_EVENT.set()
            return
        self.retry += 1
        logger.info(f'> Find {self.name} stopped, retry {self.retry}, {self.pid}')
        self.start()
    ...
    def _rotate_log(self):
        ...
        if os.path.isfile(self.log_filepath) and not os.path.isfile(backup_log_path):
            logger.info(f'Rotate log file: {self.log_filepath} => {backup_log_path}')
        ...
        if os.path.exists(to_delete_dir):
            logger.info(f'Remove old log: {to_delete_dir}')
